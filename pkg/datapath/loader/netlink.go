...
func setupDev(ifName string) error {
	link, err := netlink.LinkByName(ifName)
	if err != nil {
		log.WithError(err).WithField("device", ifName).Warn("Link does not exist")
		return err
	}
...
func setupVethPair(name, peerName string) error {
	...
	ifaces := []string{name, peerName}
	for _, iface := range ifaces {
		if err := setupDev(iface); err != nil {
			return err
		}
	}
...
func setupIpvlanSlave(nativeIface, hostIface string) error {
	...
	if err := setupDev(hostIface); err != nil {
		return err
	}
